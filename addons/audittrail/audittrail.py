...
class audittrail_objects_proxy(object_proxy):
    ...
    def get_data(self, cr, uid, pool, res_ids, model, method, recursive_level=None, parent=None):
        """
            ...
            for field in resource:
                ...
                if field_obj._type in ('one2many','many2many') and recursive_level:
                    ...
                    if self.check_rules(cr, 1, field_obj._obj, method):
                        ...
                        #kgb orig data.update(self.get_data(cr, 1, pool, resource[field], x2m_model, method))
                        ...
                        if not parent in [x2m_model.model, model.model] and resource[field]:        
                            data.update(self.get_data(cr, 1 , pool, field_resource_ids, x2m_model, method, recursive_level=recursive_level - 1, parent=model.model))
    
                        
            data[(model.id, resource_id)] = {'text':values_text, 'value': values}
        return data

    def prepare_audittrail_log_line(self, cr, uid, pool, model, resource_id, method, old_values, new_values, field_list=[], recursive_level=None, parent=None):
        """
                ...
                if self.check_rules(cr, 1, field_obj._obj, method):
                    ...
                    if not parent in [x2m_model.model, model.model] and res_ids:        
                        for res_id in res_ids:
                            lines.update(self.prepare_audittrail_log_line(cr, 1, pool, x2m_model, res_id, method, old_values, new_values, field_list, recursive_level=recursive_level - 1, parent=model.model))
            # if the value value is different than the old value: record the change
            if (key in old_values and field_name in old_values[key]['value'].keys() or key not in old_values) \
             and (key in new_values and field_name in new_values[key]['value'].keys() or key not in new_values): #KGB
    ...
    #def process_data(self, cr, uid, pool, res_ids, model, method, old_values={}, new_values={}, field_list=[]):
        ...
        for res_id in res_ids:
            ...
            for model_id, resource_id in lines:
                ...
                gname = pool.get(objmodel.model).name_get(cr, uid, [resource_id])
                name = gname and gname[0][1] or "./."
                vals = {
                    'method': method,
                    'object_id': model_id,
                    'user_id': uid,
                ...
                if lines[(model_id, resource_id)]:
                    ...
                    self.create_log_line(cr, 1, log_id, objmodel, lines[(model_id, resource_id)])
        return True
    ...
    def execute_cr(self, cr, uid, model, method, *args, **kw):
        ...
        if self.check_rules(cr,uid,model,method) and method not in ('read_group',):
