...
#                if can_compute:
    ...
    _columns = {
        ...
        'exchange_rate': fields.related('currency_id', 'rate', type='float', string='Exchange Rate', digits=(15,8)),
...
class account_move(osv.osv):
    ...
    def validate(self, cr, uid, ids, context=None):
        ...
        if context and context.get('invoice', False): #TODO FIXME ugly workaround for bug #740361, still leads to a double validation! (but only double now)
            return [move.id for move in self.browse(cr, uid, ids, context)]
