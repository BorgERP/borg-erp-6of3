            ...
            for move in too_few:
                ...
                if not new_picking:
                    ...
                    new_picking = self.copy(cr, uid, pick.id,
                            {
                                #patch'name': sequence_obj.get(cr, uid, 'stock.picking.%s'%(pick.type)),
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if move.state in ('confirmed', 'waiting'):
                ...
                if res:
                    ...
                    pickings[move.picking_id.id] = 1
                    r = res.pop(0)
                    # KGB PATCH https://launchpadlibrarian.net/108099604/stock.patch 
                    ...
                    while res:
                        r = res.pop(0)
                        #KGB move_id = self.copy(cr, uid, move.id, {'product_qty': r[0], 'location_id': r[1]})
                        product_uos_qty = self.pool.get('stock.move').onchange_quantity(cr, uid, ids, move.product_id.id, r[0], move.product_id.uom_id.id, move.product_id.uos_id.id)['value']['product_uos_qty']
