...
class sale_order(osv.osv):
  ...
  - The 'Invoice based on deliveries' choice is used to create an invoice during the picking process."""),
    ...
    #https://bugs.launchpad.net/openobject-addons/+bug/1115042
    """
    def _get_date_planned(self, cr, uid, order, line, start_date, context=None):
        dt_startdate = datetime.strptime(start_date, DEFAULT_SERVER_DATE_FORMAT)
        count = (line.delay or 0.0)
        if count > 0:
            date_planned = dt_startdate + relativedelta(list(rrule(DAILY, count=count, byweekday=('MO','TU','WE','TH','FR','SA')))[-1], dt_startdate)
        else:
...
#            qty = qty - qty % q + q
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        fpos = fiscal_position and fpos_obj.browse(cr, uid, fiscal_position, context=context) or False
        if update_tax: #The quantity only have changed
            result['delay'] = (product_obj.sale_delay or 0.0)
            #KGB result['tax_id'] = self.pool.get('account.fiscal.position').map_tax(cr, uid, fpos, product_obj.taxes_id)
