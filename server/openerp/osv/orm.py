...
class BaseModel(object):
    ...
    def _field_create(self, cr, context=None):
        ...
        if not cr.rowcount:
            cr.execute('SELECT nextval(%s)', ('ir_model_id_seq',))  # KGB db_uid cr.execute('SELECT nextval(%s)', ('ir_serial_id_seq',))  # TODO ir_model._sequence
        ...
        for (k, f) in model_fields:
            ...
            if k not in cols:
                cr.execute('select nextval(%s)', ('ir_model_fields_id_seq',))  # KGB db_uid cr.execute('select nextval(%s)', ('ir_serial_id_seq',))
                id = cr.fetchone()[0]
                vals['id'] = id
    ...
    def __init__(self, pool, cr):
        ...
        if not self._sequence:
            self._sequence = self._table + '_id_seq'  # KGB db_uid self._sequence = 'ir_serial_id_seq'
...
# TODO: set order
    ...
    def _create_table(self, cr):
        ...
        # KGB db_uid # TODO override in separate file
        # KGB db_uid cr.execute("""CREATE TABLE "%s"
        # KGB db_uid               (id bigint NOT NULL DEFAULT nextval('ir_serial_id_seq'),
        # KGB db_uid                PRIMARY KEY(id))
        # KGB db_uid            """ % (self._table,))
    ...
    def _m2m_raise_or_create_relation(self, cr, f):
        ...
        if not cr.dictfetchall():
            ...
            # KGB cr.execute('CREATE TABLE "%s" ("%s" INTEGER NOT NULL, "%s" INTEGER NOT NULL, UNIQUE("%s","%s")) ' % (m2m_tbl, col1, col2, col1, col2))
