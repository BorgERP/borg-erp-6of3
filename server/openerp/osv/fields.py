...
class selection(_column):
    ...
    def __init__(self, selection, string='unknown', **args):
        ...
        #Integer selection with 0  - start
        if (isinstance(selection, list) and isinstance(selection[0][0], int)):
                # or self.size == -1: --> functions dealing sometimes with IDs in One2Many should use NULL
            self._symbol_f = lambda x: int(x or 0)
            self._symbol_set = (self._symbol_c, self._symbol_f)
        #Integer selection with 0 - end
