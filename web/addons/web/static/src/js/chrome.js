...
openerp.web.WebClient = openerp.web.OldWidget.extend(/** @lends openerp.web.WebClient */{
    ...
    _get_version_label: function() {
        if (this.session.openerp_entreprise) {
            ...
            return _t("OpenERP - Community Ver.");
        }
    },
    ...
    start: function() {
        ...
        this.session.on_session_valid.add(function() {
            ...
            if (!self.session.openerp_entreprise) {
                ...
                self.$element.find('.oe_footer_powered').append(_.str.sprintf('<span> - <a href="mailto:support@slobodni-programi.hr?Subject=OpenERP_problem" target="_blank">%s</a></span>', version_label));
            }
            self.set_title();
        });
    },
