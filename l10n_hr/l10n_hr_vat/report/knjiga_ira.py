...
class knjiga_ira(report_sxw.rml_parse):
    ...
    def _get_company_name(self, data):
        name = self.pool.get('account.tax.code').browse(self.cr, self.uid, data['form']['chart_account_id']).company_id.name or False
        return name
    
    def _get_company_address(self, data):
        name = (self.pool.get('account.tax.code').browse(self.cr, self.uid, data['form']['chart_account_id']).company_id.partner_id.city or '') \
            + ', ' + (self.pool.get('account.tax.code').browse(self.cr, self.uid, data['form']['chart_account_id']).company_id.partner_id.address and \
                      self.pool.get('account.tax.code').browse(self.cr, self.uid, data['form']['chart_account_id']).company_id.partner_id.address[0].street or '')
        return name
    
    def _get_company_nkd(self, data):
        name = self.pool.get('account.tax.code').browse(self.cr, self.uid, data['form']['chart_account_id']).company_id.l10n_hr_base_nkd_id and \
            self.pool.get('account.tax.code').browse(self.cr, self.uid, data['form']['chart_account_id']).company_id.l10n_hr_base_nkd_id.code or False
        return name       
    
    def _get_company_vat(self, data):
        name = self.pool.get('account.tax.code').browse(self.cr, self.uid, data['form']['chart_account_id']).company_id.partner_id.vat and \
            self.pool.get('account.tax.code').browse(self.cr, self.uid, data['form']['chart_account_id']).company_id.partner_id.vat[2:] or False
        return name   
...
report_sxw.report_sxw('report.knjiga.ira', 'account.tax.code',
    'l10n_hr_vat/report/knjiga_ira.rml', parser=knjiga_ira, header=False)
